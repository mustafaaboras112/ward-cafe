...
async function main() {
 ...
 const browser = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check', '--disable-background-networking', '--disable-extensions', '--disable-sync', '--remote-debugging-port=0', '--user-data-dir=' + profile, 'about:blank'], { windowsHide: true, stdio: 'ignore' });
 let socket;
 try {
  ...
  await send('Emulation.setDeviceMetricsOverride', { width: 703, height: 1032, deviceScaleFactor: 1, mobile: false });
  ...
  for (const [name, html] of Object.entries(reports)) {
   ...
   assert.equal(dimensions.width, 703, name + ' horizontal overflow');
