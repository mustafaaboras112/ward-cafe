...
function initializeProtectedPage() {
    try {
        ...
        const workspaceName = {'admin.html':'لوحة الإدارة','accounting.html':'لوحة المحاسبة','pos.html':'نقطة البيع'}[pageName];
        ...
        lockScreen.innerHTML = `
            <div class="access-card" role="dialog" aria-modal="true" aria-labelledby="access-title">
                ...
                <span class="access-badge">بوابة فريق ورد</span>
                <h2 id="access-title">دخول ${workspaceName}</h2>
                <p>أهلاً بعودتك. أدخل رمز الموظفين للمتابعة إلى ${workspaceName}.</p>
                <form id="access-form">
                    <label for="access-code">رمز الدخول</label>
                    <input id="access-code" type="password" inputmode="numeric" autocomplete="current-password" dir="ltr" aria-describedby="access-error" required autofocus>
                    <button type="submit">الدخول إلى ${workspaceName} ←</button>
                    <small id="access-error" role="alert"></small>
                </form>
                <a class="access-back" href="index.html">العودة إلى المنيو</a>
            </div>
        `;
        document.body.appendChild(lockScreen);
        const backgroundElements = Array.from(document.body.children).filter(element => element !== lockScreen && !element.inert);
        ...
        codeInput.addEventListener('input', () => {
            codeInput.value = codeInput.value.replace(/[٠-٩۰-۹]/g, c => String(c.charCodeAt(0) - (c <= '٩' ? 1632 : 1776)));
            error.textContent = '';
        });
